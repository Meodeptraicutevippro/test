...
object.MeoHub = instance.new("ScreenGui", {
    Name = "Mo Mo Be";
    Parent = game.CoreGui;
...
pcall(function()
    if true and isfile('meohub.bin') and not getgenv().Key then
        object.TextBox.Text = readfile("meohub.bin")
...
local function SaveKey()
    local s, e = pcall(function()
        if not isfile('meohub.bin') then
            writefile('meohub.bin', GetKeyText())
        end
    end)
end
...
UIStroke.Parent = ImageButton
        local Window =
            ...
            {
                Title = "Meo Hub",
                SubTitle = "Dev By @meodeptraicutevipproreal",
                TabWidth = 160,
        ...
        function getBlueGear()
            if game.workspace.Map:FindFirstChild("MysticIsland") then
                for i, v in pairs(game.workspace.Map.MysticIsland:GetChildren()) do
                    if v:IsA("MeshPart") and v.MeshId == "rbxassetid://16137232629" then --and not v.CanCollide then
                        return v
                    end
                end
            end
        ...
        function SaveSettings2()
            local HttpService = game:GetService("HttpService")
            if not isfolder("Meo Hub") then
                makefolder("Meo Hub")
            end
            writefile("Meo Hub/" .. SaveFileName2, HttpService:JSONEncode(Settings2))
        end
        ...
        function ReadSetting2()
            local s, e =
                ...
                function()
                    ...
                    return HttpService:JSONDecode(readfile("Meo Hub/" .. SaveFileName2))
                end
            )
            if s then
                return e
